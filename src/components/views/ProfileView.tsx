...
export const ProfileView: React.FC<ProfileViewProps> = ({ currentUser, onUpdateCurrentUser }) => {
  ...
  // Handle JPEG Image Upload (Max 4MB)
  const handleImageUpload = (file: File) => {
    setProfileError('');
    setProfileSuccess('');

    // Check file extension / format (must be JPEG / JPG)
    const fileName = file.name.toLowerCase();
    const isJpeg = file.type === 'image/jpeg' || file.type === 'image/jpg' || fileName.endsWith('.jpg') || fileName.endsWith('.jpeg');
    
    if (!isJpeg) {
      setProfileError('Apenas arquivos de imagem no formato JPEG (.jpg ou .jpeg) são permitidos.');
      return;
    }

    // Check size limit (max 4MB = 4,194,304 bytes)
    const MAX_SIZE_BYTES = 4 * 1024 * 1024;
    if (file.size > MAX_SIZE_BYTES) {
      const fileSizeMB = (file.size / (1024 * 1024)).toFixed(2);
      setProfileError(`O arquivo enviado possui ${fileSizeMB} MB. O limite máximo permitido para a foto de perfil é de 4 MB.`);
      return;
    }
    ...
    reader.onload = () => {
      if (reader.result && typeof reader.result === 'string') {
        ...
        setProfileSuccess('Foto JPEG carregada com sucesso! Clique em "Salvar Dados do Perfil" para aplicar.');
      }
    };
    reader.onerror = () => {
      setProfileError('Ocorreu um erro ao ler o arquivo de imagem. Tente novamente.');
    };
              ...
              <div className="p-4 bg-slate-50 dark:bg-slate-800/60 border border-slate-200 dark:border-slate-700/80 rounded-2xl space-y-4">
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 border-b border-slate-200 dark:border-slate-700/80 pb-3">
                  <div>
                    <label className="font-bold text-slate-800 dark:text-slate-100 text-xs flex items-center gap-1.5">
                      ...
                      <span>Foto do Perfil (Upload JPEG - Máx. 4MB)</span>
                    </label>
                    <p className="text-[11px] text-slate-500 dark:text-slate-400 mt-0.5">
                      Envie uma imagem sua no formato JPEG (.jpg, .jpeg) com tamanho máximo de 4 megabytes.
                    </p>
                  </div>
                  <span className="px-2.5 py-1 bg-blue-100 dark:bg-blue-950/80 text-blue-700 dark:text-blue-300 text-[10px] font-extrabold rounded-lg uppercase tracking-wider self-start sm:self-auto border border-blue-200 dark:border-blue-900">
                    JPEG • Máx 4MB
                  </span>
                </div>
                ...
                <div className="flex flex-col sm:flex-row items-center gap-4">
                  ...
                  <div className="relative shrink-0 group">
                    ...
                    <button
                      ...
                      className="absolute inset-0 bg-slate-900/60 rounded-full opacity-0 group-hover:opacity-100 transition flex flex-col items-center justify-center text-white text-[10px] font-bold gap-1"
                      title="Alterar Foto"
                    >
                  ...
                  <div className="flex-1 space-y-2 w-full text-center sm:text-left">
                    ...
                    <div className="flex flex-wrap items-center justify-center sm:justify-start gap-2">
                      <button
                        type="button"
                        onClick={() => fileInputRef.current?.click()}
                        className="px-4 py-2 bg-blue-700 hover:bg-blue-800 text-white font-bold rounded-xl shadow-xs flex items-center gap-2 transition text-xs"
                      >
                        <Upload className="w-4 h-4" />
                        <span>Carregar Foto JPEG</span>
                      </button>

                      {profileData.photoUrl && !PRESET_AVATARS.includes(profileData.photoUrl) && (
                        <button
                          type="button"
                          onClick={() => setProfileData({ ...profileData, photoUrl: PRESET_AVATARS[0] })}
                          className="px-3 py-2 bg-slate-200 hover:bg-slate-300 dark:bg-slate-700 dark:hover:bg-slate-600 text-slate-700 dark:text-slate-200 font-semibold rounded-xl flex items-center gap-1.5 transition text-xs"
                        >
                          <Trash2 className="w-3.5 h-3.5 text-rose-500" />
                          <span>Remover Foto Customizada</span>
                        </button>
                      )}
                    </div>

                    <p className="text-[11px] text-slate-500 dark:text-slate-400">
                      Suporta apenas imagens <strong>.jpg</strong> ou <strong>.jpeg</strong> até <strong>4 MB</strong>.
                    </p>
                  </div>
                </div>

                {/* Preset Avatars Alternative */}
                <div className="pt-2 border-t border-slate-200 dark:border-slate-700/80">
                  <span className="block text-[11px] font-bold text-slate-500 dark:text-slate-400 mb-2">
                    Ou selecione um avatar padronizado:
                  </span>
                  <div className="flex items-center gap-2 overflow-x-auto pb-1">
                    {PRESET_AVATARS.map((url, idx) => (
                      <img
                        key={idx}
                        src={url}
                        alt="Avatar pré-definido"
                        onClick={() => setProfileData({ ...profileData, photoUrl: url })}
                        className={`w-9 h-9 rounded-full object-cover cursor-pointer transition ring-2 ${
